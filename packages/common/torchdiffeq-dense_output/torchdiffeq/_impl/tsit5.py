...
class Tsit5Solver(AdaptiveStepsizeODESolver):
    ...
    def __init__(
            ...
            max_num_steps=2 ** 31 - 1, dense_output=False, **unused_kwargs
    ):
        ...
        self.interpolator='_interp_eval_tsit5'
    ...
    def advance(self, next_t):
        ...
        n_steps = 0
        if self.dense_output: state_list = []
        while next_t > self.rk_state.t1:
            assert n_steps < self.max_num_steps, 'max_num_steps exceeded ({}>={})'.format(n_steps, self.max_num_steps)
            self.rk_state = self._adaptive_tsit5_step(self.rk_state)
            if self.dense_output: state_list.append(copy.deepcopy(self.rk_state))
            n_steps += 1
